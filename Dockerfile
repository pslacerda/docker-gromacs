...
RUN apt-get update && apt-get -y install build-essential wget tar zip gcc cmake automake libffi-dev
...
RUN groupadd -r $GROMACS_USER -g $GROMACS_GID && \
	...
	mkdir $GROMACS_HOME $EXPORT_DIR $PROGRAMS_ROOT && \
	chown -R $GROMACS_USER:$GROMACS_USER $PROGRAMS_ROOT $EXPORT_DIR
